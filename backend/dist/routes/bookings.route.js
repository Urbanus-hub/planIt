...
router.get("/user/:userId", authorize, authorizeRole("client", "admin"), getUserBookings);
// Get bookings for a specific service (service provider or admin)
router.get("/service/:serviceId", authorize, authorizeRole("vendor", "admin"), getServiceBookings);
// Get provider's bookings (provider themselves or admin)
router.get("/provider/:providerId/bookings", authorize, authorizeRole("vendor", "admin"), getProviderBookings);
...
router.patch("/:id/status", authorize, verifyProviderAccess, updateBookingStatus);
