...
io.on("connection", (socket) => {
    ...
    const parseCookies = (cookieString) => cookieString.split("; ").reduce((acc, cur) => {
        const [k, ...v] = cur.split("=");
        if (!k)
            return acc;
    ...
    const token = socket.handshake.auth?.token || cookies.authToken || cookies.token;
    if (token) {
        try {
            ...
            console.log(`🔐 User connected with token (userId=${decoded.id}): ${socket.id}`);
        }
        catch (err) {
    ...
    socket.on("user:join", async ({ userId, conversationId }) => {
        ...
        try {
            ...
            if (socket.data.currentConversation === conversationId) {
                console.log(`⏭️ User ${userId} already in conversation ${conversationId}, skipping`);
            ...
            if (!mongoose.Types.ObjectId.isValid(conversationId)) {
                ...
                socket.emit("conversation:error", {
                    error: "Invalid conversation ID format. Please create a conversation first.",
                });
                return;
            }
            ...
            if (conversation.participants.length !== 2) {
                console.log(`❌ Invalid conversation participants: ${conversation.participants.length}`);
                socket.emit("conversation:error", {
            ...
            if (socket.data.currentConversation) {
                ...
                console.log(`👋 User ${userId} left previous conversation: ${prevConversation}`);
            }
            ...
            console.log(`👤 User ${userId} joined TWO-PARTY conversation: ${conversationId} (with ${otherParticipant})`);
        }
        catch (error) {
    ...
    socket.on("message:send", async ({ conversationId, sender, receiver, content, attachments, relatedBooking, }) => {
        ...
        try {
            ...
            if (!participants.includes(senderStr) ||
                !participants.includes(receiverStr)) {
                socket.emit("message:error", {
                    error: "Unauthorized: sender and receiver must be the conversation participants",
                });
                return;
            }
            ...
            if (receiverSocket &&
                ...
                setTimeout(async () => {
                    await Message.updateOne({ _id: newMessage._id }, { isRead: true, readAt: new Date() });
            ...
            console.log(`📨 TWO-PARTY MESSAGE: ${senderStr} → ${receiverStr} in ${conversationId}`);
        }
        catch (error) {
