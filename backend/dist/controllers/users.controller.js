...
export const updateUser = async (req, res, next) => {
    try {
        const { name, email, phone, avatar, profileImage, profileBackground, businessName, businessDescription, businessAddress, businessLogo, serviceCategory, yearsOfExperience, businessLicense, taxId, specialties, certifications, businessHours, responseTime, city, state, website, } = req.body;
        ...
        if (businessName !== undefined)
            allowedUpdates.businessName = businessName;
        ...
        if (businessLogo !== undefined)
            allowedUpdates.businessLogo = businessLogo;
        ...
        if (taxId !== undefined)
            allowedUpdates.taxId = taxId;
        if (specialties !== undefined)
            allowedUpdates.specialties = specialties;
        if (certifications !== undefined)
            allowedUpdates.certifications = certifications;
        if (businessHours !== undefined)
            allowedUpdates.businessHours = businessHours;
        if (responseTime !== undefined)
            allowedUpdates.responseTime = responseTime;
        if (city !== undefined)
            allowedUpdates.city = city;
        if (state !== undefined)
            allowedUpdates.state = state;
        if (website !== undefined)
            allowedUpdates.website = website;
        const user = await User.findByIdAndUpdate(req.params.id, { $set: allowedUpdates }, { new: true, runValidators: true }).select("-password");
        if (!user) {
            res.status(404).json({
                success: false,
                message: "User not found",
